...
(function($) {
    function FlightIndicator(placeholder, type, options) {
        ...
        var settings = $.extend({
            ...
            pressure: 1000,
            showBox: true,
            img_directory: 'assets/libraries/jquery/flightindicators/img/'
        }, options);
        ...
        function _setPitch(pitch) {
            // alert(pitch);
            if (pitch > constants.pitch_bound) { pitch = constants.pitch_bound; } else if (pitch < -constants.pitch_bound) { pitch = -constants.pitch_bound; }
            placeholder.each(function() {
